...
async function handleVerifyOTP(request, env, corsHeaders) {
    ...
    if (existingUser) {
        ...
        return new Response(JSON.stringify({ 
            success: true, action: 'login', message: 'ورود موفقیت‌آمیز!', token, user: userData
        }), {
            status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
    } else {
        ...
        const token = generateToken(phone);
        return new Response(JSON.stringify({ 
            success: true, action: 'register', message: 'ثبت‌نام با موفقیت انجام شد! خوش آمدید.', token, user: newUser
        }), {
            status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
    }
